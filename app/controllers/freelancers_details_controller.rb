class FreelancersDetailsController < ApplicationController
  before_action :authenticate_user!
  before_action :set_freelancer_info, only: %i[edit update]
  include FreelancersDetailsHelper
  def index
  ...
  def new
    if freelancer_already_registered?
      ...
      @freelancer_info = FreelancerDetails.new
    end
  end

  def create
    @freelancer_info = FreelancerDetails.new(freelancer_params)
  ...
  def set_freelancer_info
    @freelancer_info = FreelancerDetails.find(params[:id])
  end
